...
def extract_str(val):
    """Return a plain string from a value that might be a dict, list or str."""
    if val is None:
        return ""
...
def extract_row(intv):
    """
    Turn one API intervention dict into a flat row for the spreadsheet.
    Returns a list matching XLSX_HEADERS.
    """
    ...
    # Intervention types — list of dicts or strings
    ...
    # Affected HS codes — product_id is the HS code
...
def add_sheet(wb, sheet_name, rows, product_label=None):
    ...
    # Optional product label above headers
    if product_label:
...
def build_excel(interventions_by_product):
    ...
    # All Products combined (deduplicated by intervention_id)
    seen    = set()
    all_rows = []
...
def main():
    ...
    for i, product in enumerate(PRODUCTS, 1):
        ...
        # Full intervention records for Excel
        print(f"    fetching full records ...", end=" ", flush=True)
