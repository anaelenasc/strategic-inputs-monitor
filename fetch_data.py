...
def fetch_all_interventions(hs_codes):
    ...
    while True:
        ...
        # Debug: print field names from first batch only
...
def extract_row(intv):
    ...
    # Implementing jurisdiction — try multiple possible field names
    implementer = (
    ...
    # Intervention types — try singular and plural
    types_raw = intv.get("intervention_types") or intv.get("intervention_type") or []
    if isinstance(types_raw, list):
        int_types = "; ".join(extract_str(t) for t in types_raw)
    else:
        int_types = extract_str(types_raw)

    # ALL affected HS codes stored in GTA for this intervention
...
def main():
    ...
    # Write dashboard JSON first — always saved even if Excel fails
    os.makedirs("data", exist_ok=True)
